...
from Reconstruction_3D.models.netinterface import NetInterface
from Reconstruction_3D.toolbox.cam_bp.cam_bp.functions import CameraBackProjection
    ...
    def preprocess(cls, data, mode='train'):
        ...
        for key, val in data.items():
            if key == 'rgb':
                im = val
                # H x W x 3
                im = util_img.resize(im, cls.im_size, 'horizontal')
                if mode == 'train':
                    im = util_img.jitter_colors(
                        im,
                        d_brightness=cls.rgb_jitter_d,
                        d_contrast=cls.rgb_jitter_d,
                        d_saturation=cls.rgb_jitter_d
                    )
                    im = util_img.add_lighting_noise(
                        im, cls.rgb_light_noise)
                im = util_img.normalize_colors(im)
